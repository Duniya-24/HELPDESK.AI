    ...
    return (
        <div className="space-y-10 pb-20 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight leading-none uppercase italic">Command Center</h1>
                    <p className="text-sm text-slate-400 font-bold mt-2 flex items-center gap-2 uppercase tracking-[0.2em]">
                        <Activity size={14} className="text-indigo-500" /> Intelligence Overview
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                ...
                <div className="lg:col-span-8 space-y-10">
                            ...
                            {stats.volumeTimeline.length > 0 ? (
                                <ResponsiveContainer width="100%" height="100%">
                                    <LineChart data={stats.volumeTimeline}>
                                        ...
                                        <Tooltip contentStyle={{ backgroundColor: '#fff', borderRadius: '12px', border: 'none', boxShadow: '0 20px 25px -5px rgb(0 0 0 / 0.1)' }} itemStyle={{ color: '#4f46e5', fontWeight: 900 }} />
                                        <Line type="monotone" dataKey="count" stroke="#6366f1" strokeWidth={4} dot={{ fill: '#6366f1', strokeWidth: 2, r: 4, stroke: '#fff' }} activeDot={{ r: 6, strokeWidth: 0, fill: '#4f46e5' }} />
                                    </LineChart>
                                </ResponsiveContainer>
                            ) : (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        ...
                        <Card className="p-8 border-none shadow-xl shadow-slate-200/50 rounded-[2rem] bg-white">
                            <div className="mb-8">
                                <h3 className="text-lg font-black text-slate-900 tracking-tight flex items-center gap-2 uppercase italic">
                                    <Bot size={18} className="text-indigo-500" /> Resolution Flow
                                </h3>
                            </div>
                                ...
                                {aiStats.resolutionSplit.length > 0 ? (
                                    <ResponsiveContainer width="100%" height="100%">
                                        <PieChart>
                                            ...
                                            <Legend verticalAlign="bottom" height={36} formatter={(value) => <span className="text-[9px] font-black uppercase tracking-widest text-slate-500">{value}</span>} />
                                        </PieChart>
                                    </ResponsiveContainer>
                                ) : (
                        ...
                        <Card className="p-8 border-none shadow-xl shadow-slate-200/50 rounded-[2rem] bg-white">
                            <div className="mb-8">
                                <h3 className="text-lg font-black text-slate-900 tracking-tight flex items-center gap-2 uppercase italic">
                                    <BarChart3 size={18} className="text-indigo-500" /> Category Pulse
                                </h3>
                            </div>
                            <div className="h-[250px]">
                ...
                <div className="lg:col-span-4">
                    <Card className="p-0 border-none shadow-2xl shadow-slate-200/50 rounded-[2rem] bg-white h-full relative overflow-hidden">
                        ...
                        <div className="p-6 space-y-6 overflow-y-auto max-h-[1000px] custom-scrollbar">
                            {stats.liveFeed.length > 0 ? (
                                stats.liveFeed.map((event, idx) => (
                                    <div key={idx} className="flex gap-4 group">
                                        <div className="flex flex-col items-center">
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 border-2 border-white shadow-sm z-10 ${event.type === 'create' ? 'bg-indigo-100 text-indigo-600' : event.type === 'resolve' ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`}>
                                                {event.type === 'create' ? <Inbox size={14} /> : event.type === 'resolve' ? <ShieldCheck size={14} /> : <TrendingUp size={14} />}
                                            </div>
                                            {idx !== stats.liveFeed.length - 1 && <div className="w-0.5 h-full bg-slate-100 group-hover:bg-indigo-100 transition-colors my-1"></div>}
                                        </div>
                                        <div className="pb-6">
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="font-mono text-[9px] font-black text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded uppercase tracking-tighter">{event.ticket_id.slice(0, 8)}</span>
                                ...
                                <div className="text-center py-20">
                                    <p className="text-[10px] text-slate-300 font-black uppercase tracking-[0.2em] italic">Waiting for signal...</p>
                                </div>
                            )}
                        </div>
                    </Card>
